...
from utils import file_to_name_type
...
def file_vdb_upload(username:str,file_name:str,file_type:str,file_path:pathlib.Path,base_api,api:str,model:str,db:MilvusDB,loader:FileLoader,splitter:CharacterTextSplitter):
    process=False
    try:
        embedding_size=API[api]["embeddings_size"][model]
        ...
        file_collection=MilvusCollection(db.get_collection(f"FileAbstract_{embedding_size}"),"FileAbstract",f"{embedding_size}")
        ...
        db.create_collection(f"FileBlock_{name}",get_FileBlock_fields(embedding_size))
