...
class EmbeddingsRespone:
    def __init__(self,embeddings:List[np.array],embedding_size,model,prompt_tokens, total_tokens):
...
class OpenaiAPI:
    ...
    def get_embeddings(self,texts:List[str],model,**kwargs)->EmbeddingsRespone:
        ...
        return EmbeddingsRespone(embeddings=embeddings,model=response.model,embedding_size=len(embeddings[0]),
...
class TestModelAPI:
    def __init__(self,api_key='test',base_url='http://127.0.0.1'):
        ...
        self.base_url=base_url
    def get_embeddings(self,texts:List[str],model,**kwargs)-> EmbeddingsRespone:
        return EmbeddingsRespone([np.random.random([10]) for i in range(len(texts))],embedding_size=10,model=model,prompt_tokens=10,total_tokens=10)
    def get_message(self,messages:List[dict],model:str,**kwargs) -> ChatResponse:
