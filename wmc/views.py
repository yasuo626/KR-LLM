...
from CalcSever.settings import API_URL
...
def wmc_apiurl(request):
    ...
    if request.method == "POST":
        ...
        if method=="get":
            ...
            response["state"] = 1
            response["error_info"] = f"error method"
        return JsonResponse(response)
    response["state"]=1
    response["error_info"]=f"request method {request.method}!=POST"
    return JsonResponse(response)
def set_apiurl(url):
    with open(str(API_URL.absolute()), mode='wt') as f:
        f.write(url)
def get_apiurl():
    with open(str(API_URL.absolute()), mode='rt') as f:
        url=f.read()
    return url
